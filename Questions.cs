...
namespace CDSPractical {
    public class Questions {
        ...
        public IEnumerable<int> ExtractNumbers(IEnumerable<string> source)
        {
            ...
            //check if the source has no values
            if (source != null)
            {
                foreach (var item in source)
                {
                    int number;
                    //Parse number to check if it is integer
                    chknumber = int.TryParse(item, out number);
                    if(chknumber == true)
                    {
        ...
        public string LongestCommonWord(IEnumerable<string> first, IEnumerable<string> second)
        {
            ...
            string lcommonword = string.Empty;
            if (first != null && second != null)
            {
                if (first.Count() > 0 && second.Count() > 0)
                {
                    try
                    {
                        lcommonword = first.Join(second, str1 => str1, str2 => str2, (str1, str2) => str1)
                                                       .OrderByDescending(x => x.Length)
                                                        .FirstOrDefault();
                        if(!string.IsNullOrEmpty(lcommonword))
                        {
                            return lcommonword;
                        }
                        //else
                    ...
                    {
                        Console.WriteLine("Error reading from {0}. Message = {1}",ex.Message);
                    }
                }
            }
                throw new NotImplementedException();
        ...
        public double DistanceInMiles(double km)
        {
            ...
            const double milestokilometer = 1.6;
            if (km > 0)
            {
                distanceinmiles = km / milestokilometer;
        ...
        public double DistanceInKm(double miles)
        {
            ...
            const double kmtomiles = 1.6;
            if (miles > 0)
            {
                distanceinkm = miles * kmtomiles;
                return distanceinkm;
            }
            else
            {
